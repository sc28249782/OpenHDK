...
struct FluidSynthBackend::Impl {
  fluid_settings_t* settings{}; fluid_synth_t* synth{}; fluid_player_t* player{}; ma_device device{}; bool deviceInitialized{}; bool initialized{};
...
bool FluidSynthBackend::initialize(const AudioBackendConfig& config, AudioBackendStatus& status) {
  ...
  if (config.enableDeviceOutput) {
    auto dc = ma_device_config_init(ma_device_type_playback); dc.playback.format = ma_format_f32; dc.playback.channels = 2; dc.sampleRate = config.sampleRate; dc.dataCallback = &Impl::callback; dc.pUserData = impl_.get();
    const auto result = ma_device_init(nullptr, &dc, &impl_->device);
